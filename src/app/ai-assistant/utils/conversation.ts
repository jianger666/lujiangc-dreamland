 ...
 *
 * 该函数通过POST请求处理AI模型的流式响应，支持完整的对话历史
 *
 * @param messages 要发送的消息列表
 * @param modelId 模型ID
 * @param abortControllerRef AbortController引用，用于取消请求
 * @param setStreamingState 更新流式状态的函数
 * @param conversationId 当前对话ID
 * @param conversations 所有对话列表
 * @param setConversations 更新对话列表的函数
 * @param onStreamingComplete 流式响应完成后的回调
 * @returns AbortController实例用于后续取消请求
 */
